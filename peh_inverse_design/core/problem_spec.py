...
def build_piezo_config_kwargs(problem_spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    # For coupled 3D FEM parity, electrical capacitance should use the same through-thickness
    # permittivity as the ANSYS 3D material unless an explicit capacitance override is provided.
    # The reduced_plate_constants block is retained for analytical reduced-order derivations only.
    ...
    capacitance_eps33s_f_per_m = float(
        piezoelectric.get("capacitance_eps33s_f_per_m", full_3d_eps33s_f_per_m)
    )
    isotropic_vrh = piezoelectric.get("isotropic_vrh", {}) if isinstance(piezoelectric, Mapping) else {}
    ...
    return {
        ...
        "elastic_model": str(piezoelectric.get("elastic_model", "anisotropic")),
        "voigt_order": piezoelectric.get("voigt_order", "xx,yy,zz,yz,xz,xy"),
        "isotropic_E_pa": float(isotropic_vrh.get("youngs_modulus_pa", 6.10655e10)),
        "isotropic_nu": float(isotropic_vrh.get("poisson_ratio", 0.39480)),
