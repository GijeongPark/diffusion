...
if __package__ in (None, ""):
    ...
    from ..geometry.geometry_pipeline import GeometryBuildConfig
...
def main() -> None:
    ...
    requested_mesh_preset = str(args.mesh_preset).strip().lower()
    if (
        requested_mesh_preset == "default"
        and args.substrate_layers is not None
        and args.piezo_layers is not None
        and int(args.substrate_layers) == 8
        and int(args.piezo_layers) == 3
        and args.solver_max_q2_vector_dofs is None
    ):
        requested_mesh_preset = "ansys_parity"
        print(
            "Auto-aligned mesh_preset from 'default' to 'ansys_parity' because explicit "
            "8 substrate + 3 piezo layers were requested without a quadratic-DOF cap. "
            "This keeps the direct mesh builder consistent with pipeline_runner.",
            flush=True,
        )

    preset_overrides = volume_mesh_preset_overrides(requested_mesh_preset)
    ...
    volume_config = VolumeMeshConfig(
        ...
        mesh_preset=str(requested_mesh_preset),
