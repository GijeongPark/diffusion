...
from ..geometry.geometry_pipeline import (
    GeometryBuildConfig,
...
except ModuleNotFoundError as exc:  # pragma: no cover - depends on the active Python environment
    _GMSH_IMPORT_ERROR = exc

    class _MissingGmshProxy:
        def __getattr__(self, name: str):
            raise ModuleNotFoundError(
                "No module named 'gmsh'. Select the project '.venv' interpreter or install gmsh "
                "before running meshing steps."
            ) from _GMSH_IMPORT_ERROR

    gmsh = _MissingGmshProxy()
else:
...
def _build_layered_tet_solver_mesh(
    ...
    np.savez_compressed(
        ...
        substrate_layers=np.asarray([int(volume_config.substrate_layers)], dtype=np.int32),
        piezo_layers=np.asarray([int(volume_config.piezo_layers)], dtype=np.int32),
    )
