...
from ..datasets.subset_unit_cell_dataset import subset_unit_cell_dataset
from ..solver.fenicsx_modal_solver import (
    EIGENSOLVER_BACKENDS,
    PIEZO_ELASTIC_MODELS,
...
def _to_plain(value: Any) -> Any:
    """Recursively convert a value into JSON/YAML-native Python types.

    Notebook-supplied problem-spec dictionaries may contain numpy scalars,
    numpy arrays, or tuples (for example the piezo material matrices). This
    helper converts everything to plain ``dict``/``list``/``float``/``int``/
    ``str`` so the spec serializes cleanly into both the JSON run-config
    snapshot and the YAML problem-spec snapshot.
    """
...
class PipelineConfig:
    ...
    # In-memory problem specification (materials, mechanics, electrical, geometry,
    # frf). When set, it overrides problem_spec_path and becomes the single source
    # of truth, so the notebook can hold every material/physics value inline.
    ...
    # Final normalized FRF window saved by the solver: solver_frf_points samples
    # spanning [solver_freq_ratio_min, solver_freq_ratio_max] * f_peak.
    solver_frf_points: int = 256
    solver_freq_ratio_min: float = 0.9
    solver_freq_ratio_max: float = 1.1
    solver_element_order: int = 2
    solver_store_mode_shapes: bool = False
    solver_eigensolver_backend: str = "shift_invert_cholesky"
    solver_eigensolver_fallback_backends: tuple[str, ...] | str = ()
    solver_mpi_ranks: int = 1
    piezo_elastic_model: str = "anisotropic"
    skip_existing_solver_outputs: bool = True
    solver_max_q2_vector_dofs: int | None = None
    solver_oom_fallback_element_order: int | None = None
    ...
    def __post_init__(self) -> None:
        if self.problem_spec is not None:
            object.__setattr__(self, "problem_spec", _to_plain(self.problem_spec))
        if int(self.solver_frf_points) <= 1:
            raise ValueError("solver_frf_points must be greater than 1.")
        if float(self.solver_freq_ratio_min) <= 0.0 or float(self.solver_freq_ratio_max) <= 0.0:
            raise ValueError("solver_freq_ratio_min and solver_freq_ratio_max must be positive.")
        if float(self.solver_freq_ratio_min) >= float(self.solver_freq_ratio_max):
            raise ValueError("solver_freq_ratio_min must be strictly less than solver_freq_ratio_max.")
        ...
        if self.solver_oom_fallback_element_order is not None and int(self.solver_oom_fallback_element_order) <= 0:
            object.__setattr__(self, "solver_oom_fallback_element_order", None)
        primary_backend = _normalize_eigensolver_backend(self.solver_eigensolver_backend)
        object.__setattr__(self, "solver_eigensolver_backend", primary_backend)
        fallback_backends = self.solver_eigensolver_fallback_backends
        if isinstance(fallback_backends, str):
            fallback_backends = tuple(value.strip() for value in fallback_backends.split(",") if value.strip())
        fallback_backends = tuple(_normalize_eigensolver_backend(value) for value in fallback_backends)
        object.__setattr__(
            self,
            "solver_eigensolver_fallback_backends",
            tuple(value for value in fallback_backends if value != primary_backend),
        )
        if int(self.solver_mpi_ranks) <= 0:
            raise ValueError("solver_mpi_ranks must be a positive integer.")
...
def _resolve_requested_mesh_preset(config: PipelineConfig) -> tuple[str, str | None]:
    requested = str(config.mesh_preset).strip().lower()
    if requested not in {"default", "ansys_parity"}:
        raise ValueError("mesh_preset must be 'default' or 'ansys_parity'.")

    if config.substrate_layers is None or config.piezo_layers is None:
        return requested, None
    if config.solver_max_q2_vector_dofs is not None:
        return requested, None

    explicit_layers = (int(config.substrate_layers), int(config.piezo_layers))
    if requested == "default" and explicit_layers == (8, 3):
        return (
            "ansys_parity",
            "Auto-aligned mesh_preset from 'default' to 'ansys_parity' because the explicit layer profile "
            "8 substrate + 3 piezo matches the ANSYS-parity preset. This also removes the production-only "
            "5,000,000 quadratic-vector-DOF cap. Set solver_max_q2_vector_dofs explicitly if you want to keep "
            "the old capped behavior with custom layers.",
        )
    return requested, None


def _effective_mesh_builder_settings(config: PipelineConfig) -> dict[str, object]:
    resolved_mesh_preset, _ = _resolve_requested_mesh_preset(config)
    defaults = _mesh_preset_defaults(resolved_mesh_preset)
    return {
        "mesh_preset": str(resolved_mesh_preset).strip().lower(),
...
def _build_mesh_command(
    *,
    ...
    mesh_cmd: list[str | Path] = [
        project_python,
        "-m",
        "peh_inverse_design.meshing.build_volume_meshes",
        "--unit-cell-npz",
...
def _npz_scalar(data: np.lib.npyio.NpzFile, key: str) -> object | None:
...
def _solver_outputs_exist(
    ...
    expected_element_order: int | None = None,
    expected_eigensolver_backend: str | None = None,
    expected_piezo_elastic_model: str | None = None,
) -> bool:
    ...
    try:
        ...
        return False
    if not _output_matches_mesh_provenance(
        response_path,
        mesh_provenance,
        require_layer_metadata=True,
    ):
        return False
    modal_path = _solver_modal_output_path(modal_dir, sample_id)
    if expected_element_order is not None:
        return _modal_output_matches_element_order(
            modal_path,
            int(expected_element_order),
            mesh_provenance,
            expected_eigensolver_backend=expected_eigensolver_backend,
            expected_piezo_elastic_model=expected_piezo_elastic_model,
        )
    return modal_path.exists() and _output_matches_mesh_provenance(
        modal_path,
        mesh_provenance,
        require_layer_metadata=True,
    )
...
def _assert_solver_outputs_match_requested_order(
    mesh_files: list[Path],
    modal_dir: Path,
    expected_element_order: int,
    expected_piezo_elastic_model: str,
) -> None:
    mismatches: list[str] = []
    for mesh_path in mesh_files:
        ...
        mesh_provenance = _load_solver_mesh_provenance(mesh_path)
        if not modal_path.exists():
            mismatches.append(f"sample_{sample_id:04d}: missing modal output")
            continue
        try:
            with np.load(modal_path, allow_pickle=True) as modal:
                actual = (
                    "missing"
                    if "element_order" not in modal.files
                    else str(int(np.asarray(modal["element_order"], dtype=np.int32).reshape(-1)[0]))
                )
                actual_piezo_elastic_model = (
                    "missing"
                    if "piezo_elastic_model" not in modal.files
                    else str(np.asarray(modal["piezo_elastic_model"]).reshape(-1)[0])
                )
        except Exception as exc:
            mismatches.append(f"sample_{sample_id:04d}: unreadable modal output ({exc})")
            continue
        if actual != str(int(expected_element_order)):
            mismatches.append(f"sample_{sample_id:04d}: element_order={actual}")
            continue
        try:
            actual_elastic_model = _normalize_piezo_elastic_model(actual_piezo_elastic_model)
        except ValueError:
            mismatches.append(f"sample_{sample_id:04d}: piezo_elastic_model={actual_piezo_elastic_model}")
            continue
        if actual_elastic_model != _normalize_piezo_elastic_model(expected_piezo_elastic_model):
            mismatches.append(f"sample_{sample_id:04d}: piezo_elastic_model={actual_piezo_elastic_model}")
        elif not _output_matches_mesh_provenance(modal_path, mesh_provenance, require_layer_metadata=True):
            mismatches.append(f"sample_{sample_id:04d}: modal output does not match current mesh provenance")
    if mismatches:
        joined = "; ".join(mismatches)
        raise RuntimeError(
            f"Solver outputs do not match requested element_order={int(expected_element_order)} and "
            f"piezo_elastic_model={_normalize_piezo_elastic_model(expected_piezo_elastic_model)}: {joined}. "
            "Delete stale outputs or rerun with --no-skip-existing-solver-outputs. "
            "If a lower-order OOM fallback is intentional, set --solver-oom-fallback-element-order explicitly."
        )
...
def _build_solver_inner_args(
    *,
    ...
    active_backend = (
        str(config.solver_eigensolver_backend)
        if eigensolver_backend is None
        else _normalize_eigensolver_backend(eigensolver_backend)
    )
    solver_args = ["python3", "-m", "peh_inverse_design.solver.fenicsx_modal_solver"]
    ...
    solver_args.extend(
        [
            ...
            "--frf-points",
            str(config.solver_frf_points),
            "--freq-ratio-min",
            str(config.solver_freq_ratio_min),
            "--freq-ratio-max",
            str(config.solver_freq_ratio_max),
            "--element-order",
            str(config.solver_element_order if element_order is None else int(element_order)),
            "--eigensolver-backend",
            active_backend,
            "--piezo-elastic-model",
            str(config.piezo_elastic_model),
    ...
    *,
    ...
    remaining = [
        ...
        if not _solver_outputs_exist(
            ...
            modal_dir,
            str(config.house_voltage_amplitude_convention or "peak"),
            expected_element_order=int(config.solver_element_order),
            expected_eigensolver_backend=str(config.solver_eigensolver_backend),
            expected_piezo_elastic_model=str(config.piezo_elastic_model),
        )
    ]
    if not remaining:
        return
    ...
    for mesh_path in remaining:
        ...
        try:
            ...
            solved_with_backend_fallback = False
            for fallback_backend in config.solver_eigensolver_fallback_backends:
                print(
                    f"{mesh_path.name} failed with backend={config.solver_eigensolver_backend}. "
                    f"Restarting a fresh Docker run with backend={fallback_backend}."
                )
                try:
                    _run_command(
                        _build_solver_docker_command(
                            ...
                            runtime_problem_spec_path=runtime_problem_spec_path,
                            mesh_path=mesh_path,
                            eigensolver_backend=str(fallback_backend),
                        ),
                        cwd=project_root,
                    )
                    solved_with_backend_fallback = True
                    break
                except subprocess.CalledProcessError as backend_exc:
                    mesh_exc = backend_exc
            if solved_with_backend_fallback:
                continue
...
def _write_run_config_snapshot(
    ...
    payload = {
        ...
        "effective_mesh_builder_settings": effective_mesh,
        "effective_config": {
...
def run_pipeline(config: PipelineConfig) -> PipelineArtifacts:
    ...
    problem_spec_path: Path | None = None
    if config.problem_spec is not None:
        # Notebook/API supplied an in-memory problem specification. Use it as the
        # single source of truth and snapshot it so it flows to both the mesh
        # build and the Docker solver via --problem-spec. This is what lets the
        # notebook hold every material/physics value inline.
        problem_spec = dict(_to_plain(config.problem_spec))
    ...
    if mesh_preset_alignment_note:
        print(f"Mesh preset note    : {mesh_preset_alignment_note}")
    print(
        "Solver profile      : "
        ...
        f"element_order={config.solver_element_order}, eigensolver_backend={config.solver_eigensolver_backend}, "
        f"piezo_elastic_model={config.piezo_elastic_model}, "
        f"eigensolver_fallbacks={list(config.solver_eigensolver_fallback_backends)}, "
        f"mpi_ranks={config.solver_mpi_ranks}, skip_existing={config.skip_existing_solver_outputs}, "
    ...
    if config.solver_oom_fallback_element_order is None:
        _assert_solver_outputs_match_requested_order(
            mesh_files=mesh_files,
            modal_dir=modal_dir,
            expected_element_order=int(config.solver_element_order),
            expected_piezo_elastic_model=str(config.piezo_elastic_model),
    ...
    _run_command(
        [
            project_python,
            "-m",
            "peh_inverse_design.datasets.build_response_dataset",
            "--response-dir",
            response_dir,
            "--output",
    ...
    if config.build_integrated_dataset:
        integrated_cmd: list[str | Path] = [
            project_python,
            "-m",
            "peh_inverse_design.datasets.build_integrated_dataset",
            "--unit-cell-npz",
            unit_cell_npz,
            "--response-dir",
            response_dir,
    ...
    if config.create_reports:
        ...
        _run_command(
            [
                project_python,
                "peh_inverse_design/viz/visualize_run_outputs.py",
                "--dataset",
                unit_cell_npz,
                "--mesh-dir",
                mesh_dir,
...
def _cli_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--solver-frf-points", type=int, default=256, help="Samples saved in the final normalized FRF window.")
    parser.add_argument("--solver-freq-ratio-min", type=float, default=0.9, help="Lower edge of the normalized FRF window as a fraction of f_peak.")
    parser.add_argument("--solver-freq-ratio-max", type=float, default=1.1, help="Upper edge of the normalized FRF window as a fraction of f_peak.")
    ...
    parser.add_argument(
        ...
        dest="solver_eigensolver_backend",
        default="shift_invert_cholesky",
        choices=list(EIGENSOLVER_BACKENDS),
        help="SLEPc/PETSc eigensolver backend forwarded to the FEniCSx modal solver.",
    )
    parser.add_argument(
        "--eigensolver-fallback-backends",
        "--solver-eigensolver-fallback-backends",
        dest="solver_eigensolver_fallback_backends",
        default="",
        help="Comma-separated eigensolver backends to try in fresh Docker runs after an isolated backend failure.",
    )
    parser.add_argument(
        "--solver-mpi-ranks",
        type=int,
        default=1,
        help="MPI ranks for the Docker FEniCSx solve. Values greater than 1 run through mpiexec with OMP_NUM_THREADS=1.",
    )
    parser.add_argument(
        "--piezo-elastic-model",
        default="anisotropic",
        choices=list(PIEZO_ELASTIC_MODELS),
        help="PZT elastic stiffness model forwarded to the FEniCSx modal solver.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Retry isolated Docker OOM failures once with this lower displacement order. "
            "Default 0 disables the fallback so Q2 parity runs fail instead of silently producing Q1 outputs."
        ),
    )
...
def main() -> None:
    ...
    config = PipelineConfig(
        ...
        solver_frf_points=int(args.solver_frf_points),
        solver_freq_ratio_min=float(args.solver_freq_ratio_min),
        solver_freq_ratio_max=float(args.solver_freq_ratio_max),
        solver_element_order=int(args.solver_element_order),
        solver_eigensolver_backend=str(args.solver_eigensolver_backend),
        solver_eigensolver_fallback_backends=str(args.solver_eigensolver_fallback_backends),
        solver_mpi_ranks=int(args.solver_mpi_ranks),
