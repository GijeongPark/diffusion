...
if __package__ in (None, ""):
    ...
    from peh_inverse_design.geometry.modal_surface_fields import SurfaceStrainField, available_surface_strain_fields, preferred_surface_strain_field
    from peh_inverse_design.datasets.response_dataset import normalize_voltage_amplitude_convention
else:
    from ..geometry.modal_surface_fields import SurfaceStrainField, available_surface_strain_fields, preferred_surface_strain_field
    from ..datasets.response_dataset import normalize_voltage_amplitude_convention


TOP_ELECTRODE_TAG = 105
SUBSTRATE_BOTTOM_TAG = 107
...
def _extract_surface_mesh_by_tag(
    ...
    selected = tri[tri_tags == int(facet_tag)]
    if selected.shape[0] == 0 and fallback_to_all:
        selected = tri
    if selected.shape[0] == 0:
        return points[:0], np.zeros((0, 3), dtype=np.int64)
    used_nodes = np.unique(selected.reshape(-1))
    ...
    local_tri = node_map[selected]
    return local_points, local_tri


def _extract_top_surface_mesh(mesh: dict[str, np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
    return _extract_surface_mesh_by_tag(mesh, TOP_ELECTRODE_TAG, fallback_to_all=True)


def _mesh_zoom_window(points: np.ndarray) -> tuple[np.ndarray, float]:
    span_x = float(np.max(points[:, 0]) - np.min(points[:, 0]))
    span_y = float(np.max(points[:, 1]) - np.min(points[:, 1]))
    center = np.array(
        [np.min(points[:, 0]) + 0.60 * span_x, np.min(points[:, 1]) + 0.60 * span_y],
        dtype=np.float64,
    )
    half_window = 0.08 * min(span_x, span_y)
    return center, float(half_window)


def _extract_mesh_zoom(
    points: np.ndarray,
    triangles: np.ndarray,
    window: tuple[np.ndarray, float] | None = None,
) -> tuple[np.ndarray, np.ndarray, tuple[float, float], tuple[float, float]] | None:
    if window is None:
        center, half_window = _mesh_zoom_window(points)
    else:
        center, half_window = window
...
def _plot_mesh_detail(
    ...
    if window is None:
        if points.shape[0] == 0:
            return False
        window = _mesh_zoom_window(points)
    center = np.asarray(window[0], dtype=np.float64)
    half_window = float(window[1])
    xlim = (float(center[0] - half_window), float(center[0] + half_window))
    ylim = (float(center[1] - half_window), float(center[1] + half_window))

    zoom = _extract_mesh_zoom(points, triangles, window=window)
    has_mesh = zoom is not None
    if has_mesh:
    ...
    if dataset_row is not None:
        _overlay_substrate_footprint(
            ...
            line_color=(0.05, 0.95, 0.95, 0.95),
            line_width=1.0,
        )
    return has_mesh
...
def _save_individual_plots(
    ...
    # Mesh detail: render the (solid) piezo top surface and the (voided) substrate
    # surface separately over the SAME zoom window so the void pattern is visible.
    substrate_points, substrate_triangles = _extract_surface_mesh_by_tag(
        mesh, SUBSTRATE_BOTTOM_TAG, fallback_to_all=False
    )
    detail_window = _mesh_zoom_window(points) if points.shape[0] > 0 else None
    fig, (ax_piezo, ax_substrate) = plt.subplots(1, 2, figsize=(10.6, 5.4))
    _plot_mesh_detail(
        ax_piezo,
        points,
        triangles,
        title="Piezo Top Mesh (solid)",
        show_ticks=True,
        dataset_row=dataset_row,
    ...
    _plot_mesh_detail(
        ...
        substrate_points,
        substrate_triangles,
        title="Substrate Mesh (with voids)",
        show_ticks=True,
        dataset_row=None,  # voids are shown directly as gaps; overlay would be redundant
        window=detail_window,
    )
    if detail_window is not None:
        center, half_window = detail_window
        fig.suptitle(
            f"Mesh Detail  ·  x, y ∈ [{float(center[0] - half_window):.3f}, "
            f"{float(center[0] + half_window):.3f}] m",
            fontsize=12,
        )
